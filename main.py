...
base_keyboard = InlineKeyboardMarkup([control_row, [progress_button], [close_button]])
...
async def callback_query_handler(client, callback_query):
    ...
    # ----------------- CLOSE -----------------
        ...
        try:
            ...
            await callback_query.answer("❌ Closed message.")
        except Exception as e:
...
async def seek_handler(client, message):
    ...
    seconds = int(args[1])
    if chat_id not in chat_containers or not chat_containers[chat_id]:
        await message.reply("❌ Nothing is playing to seek.")
        return

    current_song = chat_containers[chat_id][0]
    total_duration = current_song.get("duration_seconds", 0)
    if seconds >= total_duration:
        await message.reply("❌ Seek time is beyond song duration.")
        return

    try:
        # Restart playback from the seeked time
        video_url = current_song["url"]
        media_path = await vector_transport_resolver(video_url)
        await call_py.play(
            chat_id,
            MediaStream(media_path, video_flags=MediaStream.Flags.IGNORE, position=seconds*1000)
        )
        await message.reply(f"⏩ Seeked to {format_time(seconds)} in **{current_song['title']}**")
    except Exception as e:
